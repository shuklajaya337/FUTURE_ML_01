...
Merges multiple data sources and flags holiday dates:
...
def preprocess_data(train, stores, oil, holidays):
    ...
    # Join store metadata and economic indicator
...
💡 **Key concepts used**: Multi-table merges, date parsing, missing value interpolation, binary feature encoding.

---

### Step 4: Feature Engineering (`P1.py → feature_engineering()`)

Extracts time-based features and lag indicators that help the model learn patterns:
...
def feature_engineering(df):
    ...
    # Lag features — "What was the sales 7 and 14 days ago?"
    ...
    df = df.dropna()   # Remove rows with NaN from lag windows
...
💡 **Why this matters**: Lag features and rolling averages are the **backbone of time-series ML**. Without them, the model would miss temporal patterns entirely.

---

### Step 5: Model Training & Evaluation (`P1.py → train_and_evaluate()`)

Uses a **Random Forest Regressor** with time-based train/test split:
...
def train_and_evaluate(df):
    ...
    # Temporal split — last 30 days as test set (prevents data leakage)
...
💡 **Why temporal split?** Using a time-based cutoff (not random shuffle) is critical — shuffling would leak future information into training, giving falsely optimistic results.

---

### Step 6: Streamlit Web App (`app.py`)

The app wraps the entire ML pipeline into an interactive UI:

```python
@st.cache_data
def load_and_train_sales_model():
    # Generates mock data and trains the model (cached to avoid re-training)
...
- 📅 Prediction date
- 🏪 Store number (1, 2, or 3)
- 🛒 Product family (Grocery, Beverages, Produce)
- 📣 Promotional status
- 🛢️ Oil price slider

On clicking **"Generate Forecast"**, the app aligns user inputs with the trained model's feature schema and outputs the predicted demand with a confidence badge.
...
### 4. (Optional) Run the CLI Pipeline

```bash
python P1.py
```

This generates `sales_forecast_plot.png` and `feature_importance.png` in the project root.

---

## 🧪 Optional: Use Real Kaggle Data

Download the dataset from [Kaggle Store Sales Competition](https://www.kaggle.com/competitions/store-sales-time-series-forecasting) and place these files in the project root:
...
The pipeline will automatically detect and use them instead of synthetic data.
...
| File | Role |
|---|---|
| `P1.py` | Full ML pipeline: load → preprocess → feature engineer → train → evaluate → visualize |
| `app.py` | Streamlit app with integrated model and interactive UI |
| `requirements.txt` | All Python package dependencies |
| `sales_forecast_plot.png` | Generated chart: Actual vs. Predicted sales |
| `feature_importance.png` | Generated chart: Top demand drivers |
